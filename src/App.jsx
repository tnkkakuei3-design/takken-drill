...
function getWeightedPreset(catId,history){
...
export default function App(){
  ...
  const go=useCallback(async(catId,difficulty)=>{
    ...
    if(!aiMode){const p=getWeightedPreset(catId,history);if(p){setQuiz(p);setLoading(false);return;}}
    try{
      const c=CATS.find(x=>x.id===catId);const sh=[...c.topics].sort(()=>Math.random()-0.5).slice(0,3);
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:buildPrompt(c,difficulty,sh)}]})});
      if(!res.ok)throw new Error();const d=await res.json();
      const txt=d.content?.filter(b=>b.type==="text").map(b=>b.text).join("")||"";
      const parsed=JSON.parse(txt.replace(/```json|```/g,"").trim());
      if(!parsed.q||!parsed.c)throw new Error();if(!parsed.f)parsed.f=3;setQuiz(parsed);
    }catch{const p=getWeightedPreset(catId,history);if(p)setQuiz(p);else setQuiz({q:"問題の取得に失敗しました。",c:["--","--","--","--"],a:0,e:"",t:"エラー",p:"",f:1});}
